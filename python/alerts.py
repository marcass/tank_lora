...
def send_png(in_tank, period, ver):
    ret = rrdtool.graph(in_tank.rrdpath +"net.png",\
                    "--start", "end-" +period +"d",\
                    "--vertical-label=Liter",\
                    "-w 400",\
                    "-h 200",\
                    'DEF:'+in_tank.name+'='+in_tank.rrd_file+':'+ver+':AVERAGE', \
...
def gen_mulit_png(period, ver):
    ...
    for objT in inst.tank_list:
        rrd_graph_comm.append('DEF:'+objT.name+'='+objT.rrd_file+':'+ver+':AVERAGE')
