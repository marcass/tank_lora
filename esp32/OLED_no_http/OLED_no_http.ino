/*
 * Ensure TTGO LORA32-OLED V1 board selected (Tools > Board TTGO LORA32-OLED V1
 * Process goes:
 * - capture data
 * - forward to api
...
/* Install lib for SSD1306 OLED displays by:
 *  Sketch > INclude library > Manage libraries
 *  Search for SSSD1306
 *  Click "more info" for ESP8266 adn ESP32 Oled driver for SSD1306 Display by DAniel Eichman etc
 *  install version 4.0
 */  
#include "SSD1306.h"
...
void connectWifi(){
  ...
  Serial.println(ssid);
  display.drawString(0,5,"Connecting to WiFi");
  display.display();
  WiFi.mode(WIFI_STA);
...
//  display.display();
//  display.drawString(5,25,"IP " + String(ip));
//  display.display();
//  Token = getAuth();
  
  display.clear();
  display.setFont(ArialMT_Plain_10);
  display.drawString(5,0,"LoRa Receiver");
  display.display();
  SPI.begin(5,19,27,18);
  LoRa.setPins(SS,RST,DI0);

  if (!LoRa.begin(BAND)) {
    display.drawString(5,45,"Starting LoRa failed. Kill a kitten");
    while (1);
  }
...
//  }
  ...
  if ((millis() - timestamp) > COUNT_THRESH) {
    ...
    display.drawString(0, 0, time_disp);
